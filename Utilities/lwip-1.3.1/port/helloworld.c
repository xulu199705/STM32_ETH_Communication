...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define GREETING "I have gotten you in my sight!"
...
{
	struct pbuf *q;
	struct DataReceived *DataRx	= (struct DataReceived *)arg;
	int done;
	char *c;
	int i;
	...
	{        
		...
		for(q=p; q != NULL; q = q->next) 
		{
			...
			for(i=0; i<q->len && !done; i++) 
			{
				...
				if(DataRx->length < MAX_DATA_SIZE) 
				{
					DataRx->bytes[DataRx->length++] = c[i];
				}
			}
		}
		...
		for(i = 0; i < DataRx->length;i++){
			printf("%c",DataRx->bytes[i]);
		}
		printf("\r\n");

		tcp_write(pcb, DataRx->bytes, DataRx->length, TCP_WRITE_FLAG_COPY);
...
{     
  ...
  tcp_arg(pcb, mem_calloc(sizeof(struct DataReceived), 1));	
